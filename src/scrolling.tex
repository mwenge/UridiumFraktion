...
of scrolling, we could \fcode{skip} to any location on the playfield? Surely this would also be too much work?
...
we're in (which we'll call \fcode{segment}) and the other for telling us which pixel in the segment (which
we'll call \fcode{pixelX}), we have a mechanism for unlocking the ability to snap to any place in the map
...
\begin{figure}[H]
    \centering
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/pixel_dial_240.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/pixel_dial_250.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/pixel_dial_4.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/pixel_dial_14.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/pixel_dial_24.png}}
\end{figure}
\vspace{-0.4cm}
\begin{figure}[H]
    \centering
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/segment_dial_5.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/segment_dial_5.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/segment_dial_6.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/segment_dial_6.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/segment_dial_6.png}}
...
Here we are currently in \fcode{segment} 5 and incrementing our position within the segment, \fcode{pixelX},
by 10 each time. On the third increment we pass around the top of the clock and restart at zero. So an increment
of 10 from 250 brings us past 255, through 0 and up to 4. Crucially, when this happens the value for \fcode{segment} is incremented
...
\begin{figure}[H]
    \centering
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/pixel_dial_54.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/pixel_dial_30.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/pixel_dial_6.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/pixel_dial_238.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/pixel_dial_214.png}}
\end{figure}
\vspace{-0.4cm}
\begin{figure}[H]
    \centering
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/segment_dial_4.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/segment_dial_4.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/segment_dial_4.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/segment_dial_3.png}}
\hspace{0.1cm}
\raisebox{-0.3\totalheight}{\includegraphics[height=2.2cm]{src/scrolling/dials/segment_dial_3.png}}
\end{figure}
\vspace{-0.4cm}

In this example we are starting in \fcode{segment} 4 at position 54 within the segment:
\begin{figure}[H]
    \begin{adjustbox}{width=13.3cm,left}
...
Our first three decrements of \fcode{pixelX} have no effect on our segment position. But succesively
decrementing our \fcode{pixelX} value until it wraps around past zero eventually also decrements
our \fcode{segment} position by 1, moving us into \fcode{segment} 3:
\begin{figure}[H]
    \begin{adjustbox}{width=13.3cm,left}
...
When we apply \fcode{pixelX}, which now has a value of 214 according to our dials, we end up displaying
...
our \fcode{pixelX} and \fcode{segment} is the relatively compact code contained in the
routine \fcode{UpdateSegmentAndPixelX} given opposite.
...
\begin{wrapfigure}[5]{r}{3cm} % <==========================================
    ...
    \end{adjustbox}
\end{wrapfigure}
You'll notice that the operator used to move the dials is a variable called \fcode{manta\-Direction\-AndSpeed}.
This parameter not only tells us the rate at which the manta is moving but the direction in which it is moving,
whether left or right. Like \fcode{pixelX}, it has 256 possible values but in this case we treat any value over 127
...
that the routine achieves is to use \fcode{mantaDirectionAndSpeed} to update \fcode{pixelX} and detect when the dial
has moved past zero and increment or decrement \fcode{segment} when required. This relies on the behaviour of setting
the 6502 processor's \textit{carry bit} when applying \fcode{mantaDirectionAndSpeed} to our \fcode{pixelX} value. This
\textit{carry bit} is the latch that triggers our value for \fcode{segment} to move up and down as appropriate.

Up to now we haven't worried too much about how we can turn \fcode{pixelX}, or \fcode{segment}, into memory addresses
...
\begin{wrapfigure}[6]{l}{4.3cm} % <==========================================
    ...
    \end{adjustbox}
\end{wrapfigure}
when we see what is happening in the \fcode{UpdatePixelsToScroll} paragraph opposite. We
extract the last three bits (in purple) and store them in \fcode{pixelsToScroll}. This value, which will be between
...
to calculate which of the 32 characters in the \fcode{segment} we are at, our memory
position within the scroll \fcode{segment}. The trick will be
turning this number, such as '26', into an offset in the \fcode{segment} address.

Before we see how we do this, let's take stock of the mapping we want to perform. With our playfield
split into 16 segments we want to be able to take a \fcode{segment} value and map it to memory
...
We can think of each address above as representing the memory address for the top left of each \fcode{segment}.
We then need to take the top 5 'green' bits of \fcode{pixelX} and convert that into a memory position within the segment.
So starting with the simplest version of this problem, imagine we have a \fcode{segment} value of \icode{\$04} and a \fcode{pixelX} value of 0.
...
We are not just figuring out an address for \fcode{segment} by itself. We need to factor in \fcode{pixelX} too, more specifically the
first 5 bits of \fcode{pixelX}, which give us an offset between 0 and 32 within the segment.

The way we do this is by loading the last three bits of \fcode{pixelX} with a value that will affect the bit shifting behaviour
of each of the three right shifts that we perform on both \fcode{segment} and \fcode{pixelX}. We store this updated version of
\fcode{pixelX} in a new value \fcode{scrollPositionLoPtr} and also shift it to the right three times. But we use a different method
for the shifting that takes in the side-effects of the \fcode{pixelX} that manipulates its value. The key here is that the \icode{LSR}
(Logical Shift Right) operation we perform on the \fcode{segment} value has a specific side-effect on the \icode{ROR} (Rotate Right) operations
we perform on \fcode{scrollPositionLoPtr}: any time our \icode{LSR} operation pushes a \icode{0} off to the right any '1' bit rotated to the
right in \fcode{scrollPositionLoPtr} is dropped. Any time our \icode{LSR} operation pushes a \icode{1} off to the right, the '1' bits rotated
off the right in \fcode{scrollPositionLoPtr} are preserved and added back on to the front.

When we are done, it is \fcode{scrollPositionLoPtr} that will have the value we desire: the memory address for the offset within the
...
\begin{figure}[H]
  {
    ...
    \begin{adjustbox}{width=11cm,center}
      \begin{tabular}{lcc}
        \toprule
        Instruction & Accumulator & \fcode{scrollPositionLoPtr} \\
        \midrule
...
Returning to our earlier example where we had a \fcode{segment} value of \icode{\$03} and a \fcode{pixelX} value
of 214 (\icode{\$D6} in hex). We can follow the process again below. This time we can see that the '1' bits shifted off
to the right from our \fcode{segment} value in the accumulator are allowing the '1' bits rotated off \fcode{scrollPositionLoPtr}
to be preserved:
...
of the \fcode{ScrollShipSurface} and it has achieved its most important function: figuring
...
\end{figure}
\vspace{-0.8cm}
We then continue looping in \fcode{DrawNextCharacter} drawing another character:
...
We then move to the next line by jumping back to \fcode{DrawRows} and drawing
...
routine an arbitrary \fcode{segment} and \fcode{pixelX} value. The entire process
