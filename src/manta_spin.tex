...
\begin{figure}[H]
    ...
    \begin{adjustbox}{width=13cm,center}
      \includegraphics[width=13cm]{src/manta_spin/manta_spin.png}%
    \end{adjustbox}
\end{figure}
\vspace{-0.5cm}

The first phase of the spin -
\raisebox{-0.2\totalheight}{\includegraphics[height=0.5cm]{src/manta_spin/manta_spin_last_two_2.png}}
...
\raisebox{-0.2\totalheight}{\includegraphics[height=0.5cm]{src/manta_spin/manta_spin_last_two_3.png}}
...
Let's take a look at how the animation is implemented in the routine responsible
for spinning the manta. It's thankfully very compact and easy to digest.
We would expect it to iterate over the sprites from \fcode{MANTA\_0} to \fcode{MANTA\_15}, which is 
exactly what it does - as we noted, all that matters to it is that the sprites are ordered
contiguously in RAM. Selecting the next sprite for preparation (\fcode{GetCurrentSprite})
and display (\fcode{DisplaySprite}) is a simple matter of incrementing the index
\fcode{currentSpriteValue} that points to the current sprite until we reach \fcode{MANTA\_16}, 
at which point we reset our \fcode{currentSpriteValue} back to \fcode{MANTA\_0}.
...
that when \fcode{frameCount} has reached zero, and the animation is over, that we finish with
the Manta facing upwards:
\raisebox{-0.2\totalheight}{\includegraphics[height=0.5cm]{src/manta_spin/MANTA1.png}}?
The answer lies in the initial value that we set for \fcode{frameCount}. This is \icode{\$BF} or 191
in decimal: 
\begin{lstlisting}
    LDA #$BF         ; Store 191 in the A register.
    STA frameCount   ; Store it as our frameCount.
\end{lstlisting}
A starting value of 191 for \fcode{frameCount} gives us 65 keyframes of animation in total until
we reach 256 and roll around, like a clock, back to zero. With 16 frames for each complete spin of the Manta
this allows us 4 complete spins (64 frames) with one left over before \fcode{frameCount} will roll over to
zero. It is this left over frame that will push us past \fcode{MANTA\_00} -  
\raisebox{-0.2\totalheight}{\includegraphics[height=0.5cm]{src/manta_spin/MANTA.png}}
- and finish at the desired spot of \fcode{MANTA\_1} - 
\raisebox{-0.2\totalheight}{\includegraphics[height=0.5cm]{src/manta_spin/MANTA1.png}}.
...
\begin{figure}[H]
    \centering
    \begin{adjustbox}{width=13cm,center}
      \includegraphics[width=13cm]{src/manta_spin/manta_spin_left.png}%
    \end{adjustbox}
\end{figure}
\vspace{-0.6cm}
