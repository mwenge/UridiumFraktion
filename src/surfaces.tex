...
working from the bottom up. Pair the tile values in each array with their corresponding
...
GoToNextStrip   
  LDA currentLevelSurfaceLoPtr ; Get position of current strip on dreadnought.
  ADC #$01                     ; Increment by one in order to draw ..
  STA currentLevelSurfaceLoPtr ; .. the next strip in the next position to the right.
  LDA currentLevelSurfaceHiPtr ; Get the 'end' of the dreadnought.
  CMP #>endOfSurfaceData       ; Have we reached it yet?
  BCS ReadPlacedStructures     ; If so, we've finished drawing the surface section.
...
Our next step is to see how we go about the finishing phase required to create a completed dreadnought. How
