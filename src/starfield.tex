...
to the left for every pixel that the dreadnought moves to the right, using the number of pixels calculated for hardware scroll to determine how far to move it. The diagrams
below give us an idea of how this might be achieved. We update the byte responsible for defining the star with a
value that has the effect of moving it to the left along the horizontal.
...
This will have the desired effect for the small star: moving it to the left as the dreadnought moves to the right
(and vice versa) ensuring that it remains in a fixed position while the rest of the 
surface appears to move along above it. To perform the same effect for the larger star it is just a question
