...
In the routine above it is the subroutine \fcode{WriteARowOfStars} that is doing this heavy lifting, so that is where we need
to look to get an idea of how the distribution of the stars on a line is achieved in practice. What we find is that
for each character in the line we pluck a value from a pool of random values stored in the \fcode{randomData} array. If the value
...
of stars at the top of the screen by adjusting the our starting point in \fcode{ramLoPtr} and \fcode{ramHiPtr}:
...
its information in 3 separate arrays. The first two arrays are \fcode{starDataLoPtrArray} and \fcode{starDataHiPtrArray}.
Together these give us the location of each star on the screen. The third array is \fcode{starsBehindDreadnought}. This
...
There is one final twist in \fcode{AddStarsBehindDreadnought}. As the dreadnought scrolls
...
\begin{figure}[H]
    ...
    \begin{adjustbox}{center}
      ...
      \hspace{0.5cm}
      \raisebox{1.8\totalheight}{\includegraphics[height=0.6cm]{src/baydoor_sprites/arrow.png}}
      \hspace{0.3cm}
...
into \fcode{parallaxOffsetsForStars} will pick an update for the star that will offset it five pixels to 
the left.
...
value in \fcode{pixelsToScroll}. (We will see how the value in \fcode{pixelsToScroll} is calculated in the next chapter).
\begin{lstlisting}
...
element in the \fcode{pixelsToScroll} array, which is the default, desired position of the star on the extreme
